...
class ComputeUnit {
 public:
  ComputeUnit(std::string shaderSrc) : shaderSrc_(shaderSrc) {}
  ...
  /*
    void createComputePipeline() {
      ...
      VK_CHECK_RESULT(vkCreateShaderModule(
          device_, &createInfo, nullptr, &computeShaderModule_));
      ...
      pipelineLayoutCreateInfo.pSetLayouts = &descrSetLayout_;
  ...
  */
  ...
  void createComputePipelineWithCompilation(
      const VkDescriptorSetLayout& descrSetLayout) {
    COUT_FLF;
    std::cout << "-----" << std::endl;
    std::cout << shaderSrc_ << std::endl;
    std::cout << "-----" << std::endl;
    ...
    VkShaderModuleCreateInfo createInfo = {};
    {
      ...
      createInfo.codeSize = 4 * shaderSpvCode.size();
    }
    auto& device = vkContext->device_;
    COUT_FLF;
    VK_CHECK_RESULT(vkCreateShaderModule(
...
void VulkanVulkanTensor::setDataFromHost(const float* inputData) {
  ...
  const char* shaderSrcCStr = at::native::vulkan::vulkan_nchw_buf_to_tex_glsl;
  size_t shaderSrcLength = std::strlen(shaderSrcCStr);
  ...
  ComputeUnit computeUnit{std::string{shaderSrcCStr, shaderSrcLength}};

  // createComputePipeline();
  computeUnit.createComputePipelineWithCompilation(descrSetLayout);
  COUT_FLF;
...
void VulkanVulkanTensor::copyDataToHost(float* output) {
  ...
  const char* shaderSrcCStr = at::native::vulkan::vulkan_tex_to_nchw_buf_glsl;
